...
# ────────────────────────────────────────────────────────────────
# 2️⃣ Daily Incremental Collection1
# ────────────────────────────────────────────────────────────────
def daily_incremental_collection_task(**kwargs) -> dict:
    ...
    if is_hourly:
        ...
        # daily/수동: 기존 로직 유지 (execution_date → 오늘 06:30)
    ...
    # 마지막 수집 시점 확인 (Variable 있으면 항상 여기서 구간 설정 → daily→hourly 전환 시 06:30 이후 구간 누락 없음)
    last_extract_time_str = Variable.get(INCREMENT_KEY, default_var=None)

    if last_extract_time_str:
        # Variable이 있으면 무조건 (Variable+1초) ~ target_end_date 수집. '직전 1시간' 로직은 Variable 없을 때만 사용.
        ...
        # 마지막 수집 시점이 없으면: hourly는 직전 1시간, daily는 전날 06:30부터
        if is_hourly:
...
# ────────────────────────────────────────────────────────────────
with DAG(
    dag_id="sss_ipp_so_incremental",
    default_args=DEFAULT_ARGS,
    schedule_interval=None,  # 매일 실행
    start_date=datetime(2025, 1, 1),
    catchup=False,
    tags=["JJ","raw", "IP","Plan", "bronze layer", "incremental", "production"]
) as dag:
